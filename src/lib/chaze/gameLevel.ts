...
function loadLevelData(level: number) {
	...
	for (let i = mapLayers.length - 1; i > -1; i--) {
		...
		if (isForeground) foregroundTileLayer = mapLayer;
		...
		for (let x = levelSize.x; x--; )
			for (let y = levelSize.y; y--; ) {
				...
				const objectPos = pos.add(LJS.vec2(0.5));
...
function proceduralMap() {
	...
	const SCALE = 0.125; // adjust for smoothness
	...
	for (let x = GRID; x--; )
		for (let y = GRID; y--; ) {
			...
			let tile = isEdge(x, y, GRID)
				? Game.spriteAtlas.wall
				: n > WALL_THRESHOLD
					? Game.spriteAtlas.wall
					: Game.spriteAtlas.floor;
