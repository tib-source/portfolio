...
export function getAvailablePointsNearObjectBFS(o: LJS.EngineObject, radius: number) {
	...
	while (queue.length > 0) {
		...
		if (curr) {
			if (visited.has(key(curr))) continue;
...
export function getNearbyObjectBFS<T extends GameObject>(
	o: GameObject,
	radius: number,
	ObjectType: new (...args: any[]) => T,
	dirs?: LJS.Vector2[],
	optimiseForObject?: GameObject
) {
	...
	while (queue.length > 0) {
		...
		if (curr) {
			...
			if (seen.has(gridKey(curr))) continue;
			...
			for (let n of neighbors) {
				if (
					optimiseForObject &&
					optimiseForObject.pos.distance(n) > optimiseForObject.pos.distance(curr)
				) {
					continue;
				}
...
export function aStarPathFinder(
	start: LJS.Vector2,
	goal: LJS.Vector2,
	givenDirs?: LJS.Vector2[],
	optimiseForObject?: GameObject
) {
	grid = generateGrid(true);

	start = start.floor().add(LJS.vec2(0.5));
	goal = goal.floor().add(LJS.vec2(0.5));
	...
	while (queue.size > 0) {
		...
		for (let dir of dirs) {
			...
			if (!node || seen.has(node) || !node.walkable || LJS.tileCollisionRaycast(curr.pos, node.pos))
				continue;
