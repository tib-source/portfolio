...
export default class Chaze extends Scene {
	...
	renderPost() {
		...
		LJS.engineFontImage.drawTextScreen(
			`Ammo : ${'*'.repeat(Game.player.ammoCount)}`,
			LJS.vec2(30, 30),
			20,
			false,
			LJS.WHITE
		...
		LJS.drawRect(
			LJS.vec2(150, 60).add(LJS.vec2(-120 + x / 2, 0)),
			LJS.vec2(x, 30),
			LJS.GREEN,
			undefined,
			undefined,
		...
		LJS.engineFontImage.drawTextScreen(
			...
			LJS.vec2(Game.GAME_WIDTH - 200, 30),
			20,
			true,
			LJS.WHITE
		);
		...
		if (Game.player.boostTimer?.active()) {
			LJS.engineFontImage.drawTextScreen(
				`BOOST : ${Game.player.boostType}`,
				LJS.vec2(Game.GAME_WIDTH - 200, 60),
				20,
				true,
				LJS.WHITE
			);
