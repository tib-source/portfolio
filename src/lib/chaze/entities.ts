...
export class GameObject extends LJS.EngineObject {
	...
	damage(damage: number, damagingObject: GameObject) {
		...
		for (const child of this.children) child.damageTimer && child.damageTimer.set();
	...
	kill(damagingObject: GameObject) {
		this.destroy();
	}
	...
	render() {
		if (this.displayHUD) {
			...
			let healthColor = this.isBoosted(BOOST_TYPE.SHIELD)
				? LJS.BLUE
				: this.isBoosted(BOOST_TYPE.POISON)
					? LJS.PURPLE
					: this.isEnemy
						? LJS.RED
						: LJS.GREEN;
			LJS.drawRect(this.pos.add(vec2(0, base)), vec2(1.05, 0.2), LJS.BLACK);
			LJS.drawRect(this.pos.add(vec2(-0.5 + x / 2, base)), vec2(x, 0.15), healthColor);

			if (!(this instanceof Player) && Game.debugEnemyInfo) {
				LJS.drawText(`STATE: ${this.state}`, this.pos.add(vec2(0, base + 0.25)), 0.25, LJS.WHITE);
				LJS.drawText(`AMMO: ${this.ammoCount}`, this.pos.add(vec2(0, base + 0.5)), 0.25, LJS.WHITE);
				LJS.drawText(`ID: ${this.getName()}`, this.pos.add(vec2(0, base + 0.75)), 0.25, LJS.WHITE);
			}
		}
	...
	debuf() {
		this.additiveColor = LJS.hsl(0.15, 1, 0.5);
	}
		...
		if (LJS.mouseIsDown(0)) {
			if (Game.debugPathFinderWithPlayer) {
				...
				for (let node of path) {
					LJS.debugPoint(node.add(LJS.vec2(0.5)), LJS.RED, 0.1);
				}
			} else {
				this.shoot(angle);
			}
...
export class Enemy extends GameObject {
		...
		switch (this.state) {
			...
			case ENEMY_STATE.ATTACK:
				...
				if (this.health == this.fullHealth * LJS.rand(0.1, 0.35)) {
					if (!this.haveFled) this.state = ENEMY_STATE.FLEE;
				}
				break;
	...
	patrol() {
		...
		if (!this.nextPos || this.pos.distance(this.nextPos) <= 0.5) {
			this.nextPos = AI.pickRandomPoints(this.wayPoints);
		}

		if (this.nextPos) {
			this.velocity = this.velocity.add(this.nextPos.subtract(this.pos).normalize(0.05));
			if (LJS.debugOverlay || Game.debugEnemyPathFinder) LJS.debugPoint(this.nextPos, LJS.RED, 0.1);
		}
	}
...
export class Bishop extends Enemy {
	...
	update(): void {
		if (this.state != ENEMY_STATE.IDLE) this.applyAuraBuffs();
		super.update();
	}
...
export class Bullet extends LJS.EngineObject {
	...
	constructor(
		...
		this.drawSize = vec2(0.15, 0.15);
		this.size = vec2(0.15, 0.15);
	...
	update() {
		...
		if (this.range < 0) {
			new LJS.ParticleEmitter(
				...
				50,
				3.14,
				Game.spriteAtlas.pawn, // pos, emit info, tileInfo
				LJS.rgb(1, 1, 0.1),
				LJS.rgb(1, 1, 1), // colorStartA, colorStartB
				LJS.rgb(1, 1, 0.1, 0),
				LJS.rgb(1, 1, 1, 0), // colorEndA, colorEndB
				0.1,
				0.5,
				0.1,
				0.05,
				0, // particleTime, sizeStart, sizeEnd, speed, angleSpeed
				1,
				1,
				0.5,
				3.14,
				0.1, // damping, angleDamping, gravityScale, cone, fadeRate,
				0.5,
				true,
				true // randomness, collide, additive, randomColorLinear
			);
	...
	kill() {
		...
		const emitter = new LJS.ParticleEmitter(
			...
			0.1,
			100,
			0.5, // pos, angle, size, time, rate, cone
			undefined, // tileInfo
			LJS.rgb(1, 1, 0),
			LJS.rgb(1, 0, 0), // colorStartA, colorStartB
			LJS.rgb(1, 1, 0),
			LJS.rgb(1, 0, 0), // colorEndA, colorEndB
			0.2,
			0.2,
			0,
			0.1,
			...
			0.5,
			3.14,
			0.1, // damp, angleDamp, gravityScale, particleCone, fade,
			0.5,
			true,
			true // randomness, collide, additive
		);
